...
public class PushMojo extends AbstractIacMojo {
	 ...
	 *
	 * In general each project generates just one artifact, however it may have
	 * other artifacts as dependencies.
	 * If this maven plugin has been configured to include dependencies
	 * ({@link #includeDependencies} is true), then the target artifact, together
	 * with all dependencies
	 * including any transitive dependencies will be collected into a Collection
	 * object and passed as parameter to this method.
	 *
	 * In case {@link #includeDependencies} is false, then the Collection will
	 * contain just the target artifact (the one that is being generated by this
	 * maven project).
	 *
	 * We would loop through the whole collection of artifacts and import (push)
	 * each of them individially to the remote vROps server.
	 *
	 * @param allArtifacts The collection of artifacts to the imported (pushed) to
	 *                     the remote vROps server.
	 * @throws MojoExecutionException In case the maven plugin configuration is
	 *                                wrong or any other operation goes wrong.
	 */
	private void importArtifacts(Collection<Artifact> allArtifacts) throws MojoExecutionException {
		...
		for (String type : artifactsByType.keySet()) {
			...
			if (pkgType != null) {
				try {
					List<Package> packages = artifactsByType.get(type).stream().map(PushMojo::packageFromArtifact)
					...
					PackageStore<?> store = getConfigurationForType(PackageType.fromExtension(type))
							...
							.orElseThrow(() -> new ConfigurationException(
									"Unable to find PackageStore based on configuration. "
					...
					getLog().error(e);
					throw new MojoExecutionException(e, "Error processing configuration",
	...
	/**
	 * Implements the import/push use case to import package(s) into a remote vROps
	 * server.
	 * In all cases, the target artifact (the one that is generated by this maven
	 * project) will be imported/pushed. The parameter {@link #includeDependencies}
	 * will also
	 * be checked. If it is true, then additionally all dependencies including
	 * transitive dependencies will be imported/pushed to the remote vROps server.
	 * 
	 * @throws MojoExecutionException In case there is failure in the maven plugin
	 *                                execution.
	 * @throws MojoFailureException   In case there is failure in the Mojo (maven
	 *                                plugin).
	 */
	@Override
