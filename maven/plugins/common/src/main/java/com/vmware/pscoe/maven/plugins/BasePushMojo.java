...
public class BasePushMojo extends AbstractIacMojo {
	...
	protected static Package packageFromArtifact(Artifact artifact) {
	...
	private void importArtifacts(Collection<Artifact> allArtifacts) throws MojoExecutionException {
		...
		for (Map.Entry<String, List<Artifact>> type : artifactsByType.entrySet()) {
			...
			try {
				List<Package> packages = artifactsByType.get(type.getKey()).stream().map(BasePushMojo::packageFromArtifact)
