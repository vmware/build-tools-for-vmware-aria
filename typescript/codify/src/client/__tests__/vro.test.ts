...
describe('vRO Client', () => {
  ...
  it('Get Client', async () => {
    ...
    const client = await getVroClient({ hostname: 'foo', username: 'bar', password: 'baz' });
  ...
  it('Get client with valid cached credentials', async () => {
    ...
    const client = await getVroClient({ hostname: 'foo', username: 'bar@system domain', password: 'baz' });
