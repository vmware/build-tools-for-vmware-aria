...
@Workflow({
	...
	attributes: {
		...
		att2: { type: "Array/number" },
                att3: { type: "string", bind: true, value: "Some/Path/To/ConfigurationEl/variable1" }
	}
})
export class TestWorkflow4 {
	action1(param1: string, param2: string, att3: string, @Out att1: string, @Out att2: number[]): void {
		System.log(`[action1] param1='${param1}', param2='${param2}', att3=${att3}`);
		new TestClass1();
		new TestClass2();
		att1 = "Att1";
		att2 = [31, 17, 44];
	...
	action2(param3: number, att1: string, att2: number[], att3: string): void {
		System.log(`[action2] param3='${param3}', att1='${att1}', att2='${(att2 || []).map(x => x.toString()).join(",")}', att3=${att3}`);
	}
