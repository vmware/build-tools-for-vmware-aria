...
import { WorkflowDescriptor, parseWorkflowXml, printWorkflowXml, WorkflowParameter, WorkflowAttribute, WorkflowItemBinding, WorkflowItem } from "./workflow";
...
export function getSagaTransformer(file: FileDescriptor, context: FileTransformationContext) {
	...
	function loadSaga(): SagaDescriptor {
		...
		saga.description = saga.description || "";
	...
	function buildWorkflow(saga: SagaDescriptor): WorkflowDescriptor {
		...
		if (!saga.omitOutput) {
			buildWorkflowOutput(workflow, saga.omitSagaOutput, saga.output);
		}
	...
	function buildWorkflowOutput(workflow: WorkflowDescriptor, omitSagaOutput: boolean, output?: Record<string, OutputParameter>): void {
	...
	function buildWorkflowTasks(workflow: WorkflowDescriptor, saga: SagaDescriptor): void {
		...
		function buildItem(saga: SagaDescriptor, taskName: string, task: TaskDescriptor, index: number): void {
			...
			if (task.workflow && !task.if) {
				...
				items.push({
					...
					"catch-name": getRollbackTaskName(saga, task, isFirst, executeItemId, finishRollbackItemId),
					"throw-bind-name": ATT_ERROR,
					position: {
						x: posX,
						y: 55.40909090909091,
		...
		function getRollbackTaskName(saga: SagaDescriptor, task: TaskDescriptor, isFirst: boolean, executeItemId: number, finishRollbackItemId: number): string {
			if (saga.rollbackCurrentTask || task.rollbackCurrentTask) {
				// return the rollback item for the current task
				return getItemName(executeItemId + 1);
			}
			if (isFirst) {
				// return the final rollback item
				return getItemName(finishRollbackItemId);
			}
			// return the rollback item for the previous task
			return getItemName(executeItemId - 1);
		}
	}
	...
	function buildExecuteScript(saga: SagaDescriptor, taskName: string, task: TaskDescriptor): string {
		let builder = new StringBuilderClass();
	...
	function buildRequireModuleScript(builder: StringBuilderClass, saga: SagaDescriptor, taskName: string): void {
	...
	function buildWorkflowExecutor(builder: StringBuilderClass, workflowId: string, conditionalVar: string, rollback: boolean): StringBuilderClass {
		builder.append(`if (${ATT_STATE}["${conditionalVar}"]) {`).appendLine();
		builder.indent();
	...
	function mergeWorkflow(workflow: WorkflowDescriptor, saga: SagaDescriptor): WorkflowDescriptor {
		...
		workflow.description = saga.description || "";
		workflow.position = {
			x: 105,
			y: 45.90909090909091,
		};
		...
		buildWorkflowInput(workflow, saga.input);
		buildWorkflowOutput(workflow, saga.omitOutput, saga.output);
		buildWorkflowAttributes(workflow, hasWorkflowTasks);
		buildWorkflowTasks(workflow, saga);
		buildWorkflowPresentation(workflow, saga);
