describe('vRO Server API', () => {
    ...
    describe('Configuration elements are supported', () => {
        ...
        it('Root Configuration element categories list has expected size', () => {
            ...
            expect(
                Server.getAllConfigurationElementCategories().length
            ).toEqual(1);
        });
        it('Fetching existing configuration category by path', () => {
            const category =
                Server.getConfigurationElementCategoryWithPath('PSCoE/Tests');
            expect(category).toBeDefined();
            expect(category).not.toBeNull();
        });
        it('Fetching non-existing configuration category by path', () => {
            const category =
                Server.getConfigurationElementCategoryWithPath('Foo');
        ...
        it('Configuration category has available elements', () => {
            ...
            expect(category.configurationElements[0]).toBeInstanceOf(
                ConfigurationElement
            );
            expect(category.allConfigurationElements[0]).toBeInstanceOf(
                ConfigurationElement
            );
        });
        it('Create and delete configuration categories', () => {
            ...
            Server.createConfigurationElement(
                'PSCoE/ToBeDeleted',
                'Test Element'
            );
        ...
        it('Create and delete configuration elements', () => {
            ...
            const byRef = Server.createConfigurationElement(
                category,
                'test-by-cat-ref'
            );
            ...
            // ... with category name
            const byName = Server.createConfigurationElement(
                'PSCoE/Tests',
                'test-by-cat-name'
            );
            ...
            expect(
                category.configurationElements.filter(
                    (e) => e.name === 'test-by-cat-ref'
                ).length
            ).toEqual(0);
            expect(
                category.allConfigurationElements.filter(
                    (e) => e.name === 'test-by-cat-ref'
                ).length
            ).toEqual(0);
        ...
        it('Manipulate configuration element values', () => {
            ...
            function testElement(confEl) {
                ...
                if (confEl.getAttributeWithKey('stringAttr') !== null) {
                    expect(
                        confEl.getAttributeWithKey('stringAttr')
                    ).toBeInstanceOf(Attribute);
                    expect(
                        confEl.getAttributeWithKey('stringAttr').value
                ...
                if (confEl.getAttributeWithKey('numberAttr') !== null) {
                    expect(
                        confEl.getAttributeWithKey('numberAttr')
                    ).toBeInstanceOf(Attribute);
                    expect(
                        confEl.getAttributeWithKey('numberAttr').value
                    ).toEqual(1);
                }
                // ... booleans
                if (confEl.getAttributeWithKey('booleanAttr') !== null) {
                    expect(
                        confEl.getAttributeWithKey('booleanAttr')
                    ).toBeInstanceOf(Attribute);
                    expect(
                        confEl.getAttributeWithKey('booleanAttr').value
                    ).toEqual(true);
                }
                // ... RESTHost - no value supported
                if (confEl.getAttributeWithKey('restHost') !== null) {
                    expect(
                        confEl.getAttributeWithKey('restHost')
                    ).toBeInstanceOf(Attribute);
                    expect(
                        confEl.getAttributeWithKey('restHost').value
                    ).toEqual(null);
                }
                // ... Array of strings
                ...
                if (confEl.getAttributeWithKey('strArray') !== null) {
                    expect(
                        confEl.getAttributeWithKey('strArray')
                    ).toBeInstanceOf(Attribute);
                    expect(
                        confEl.getAttributeWithKey('strArray').value
                    ).toEqual(['str1', 'str 2', 'str #3']);
                }
                // ... Array of booleans
                if (confEl.getAttributeWithKey('boolArray') !== null) {
                    expect(
                        confEl.getAttributeWithKey('boolArray')
                    ).toBeInstanceOf(Attribute);
                    expect(
                        confEl.getAttributeWithKey('boolArray').value
                    ).toEqual([true, false, true]);
                }
                // ... Array of numbers
                if (confEl.getAttributeWithKey('numArray') !== null) {
                    expect(
                        confEl.getAttributeWithKey('numArray')
                    ).toBeInstanceOf(Attribute);
                    expect(
                        confEl.getAttributeWithKey('numArray').value
                    ).toEqual([1, 2, 3]);
                }
                ...
                expect(
                    confEl.getAttributeWithKey('customStrAttr').value
                ).toEqual('value');

                // ... number
                confEl.setAttributeWithKey('customNumAttr', 1, 'number');
                expect(
                    confEl.getAttributeWithKey('customNumAttr').value
                ).toEqual(1);

                // ... boolean
                confEl.setAttributeWithKey('customBoolAttr', true, 'boolean');
                expect(
                    confEl.getAttributeWithKey('customBoolAttr').value
                ).toEqual(true);

                // ... Array of strings
                confEl.setAttributeWithKey('customStrArr', ['value'], 'string');
                expect(
                    confEl.getAttributeWithKey('customStrArr').value
                ).toEqual(['value']);

                // ... Array of numbers
                confEl.setAttributeWithKey('customNumArr', [1], 'number');
                expect(
                    confEl.getAttributeWithKey('customNumArr').value
                ).toEqual([1]);

                // ... Array of booleans
                confEl.setAttributeWithKey('customBoolArr', [true], 'boolean');
                expect(
                    confEl.getAttributeWithKey('customBoolArr').value
                ).toEqual([true]);
            }
            ...
            const localCat =
                Server.getConfigurationElementCategoryWithPath('PSCoE/Tests');
            const localConf =
                localCat.configurationElements != null
                    ? localCat.configurationElements.find(
                          (e) => e.name === 'Sample Config'
                      )
                    : [];
            ...
            const runtimeElement = Server.createConfigurationElement(
                localCat,
                'test-runtime-conf-element'
            );
            runtimeElement.setAttributeWithKey(
                'stringAttr',
                'String value',
                'string'
            );
    ...
    describe('Resource elements are supported', () => {
        ...
        it('Fetching existing resource category by path', () => {
            const category =
                Server.getResourceElementCategoryWithPath('PSCoE/Tests');
            expect(category).toBeDefined();
            expect(category).not.toBeNull();
        });
        ...
        it('Resource category properties are available', () => {
            const pscoeCategory =
                Server.getResourceElementCategoryWithPath('PSCoE');
        ...
        it('Resource element category has available elements', () => {
            ...
            // Elements are instances of the expected class
            expect(category.resourceElements[0]).toBeInstanceOf(
                ResourceElement
            );
            expect(category.allResourceElements[0]).toBeInstanceOf(
                ResourceElement
            );
        });
        it('Read resource elements', () => {
            ...
            const element =
                category.resourceElements != null
                    ? category.resourceElements.find(
                          (e) => e.name === 'text.txt'
                      )
                    : null;
            expect(element.mimeType).toEqual('text/plain');
            expect(element.getContentAsMimeAttachment()).toBeInstanceOf(
                MimeAttachment
            );
            expect(element.getContentAsMimeAttachment().content.trim()).toEqual(
        ...
        it('Update resource elements', () => {
            ...
            const element = Server.createResourceElement(
                category,
                'test-elem1'
            );
            expect(
                null == element.getContentAsMimeAttachment().content
            ).toBeTruthy();
            ...
            expect(element.getContentAsMimeAttachment().content).toEqual(
                'Example content'
            );
            expect(element.getContentAsMimeAttachment().mimeType).toEqual(
                'text/plain'
            );
        ...
        it('Create and delete resource elements', () => {
            ...
            // ... with category reference
            const byRef = Server.createResourceElement(
                category,
                'test-by-cat-ref',
                content
            );
            ...
            expect(byRef.getContentAsMimeAttachment()).toBeInstanceOf(
                MimeAttachment
            );
            expect(byRef.getContentAsMimeAttachment().content).toEqual(
                'Example content'
            );
            expect(byRef.getContentAsMimeAttachment().mimeType).toEqual(
                'text/plain'
            );
            ...
            // ... with category name
            const byName = Server.createResourceElement(
                'PSCoE/Tests',
                'test-by-cat-name',
                content,
                'text/plain'
            ...
            expect(byName.getContentAsMimeAttachment()).toBeInstanceOf(
                MimeAttachment
            );
            expect(byName.getContentAsMimeAttachment().content).toEqual(
                'Example content'
            );
            expect(byName.getContentAsMimeAttachment().mimeType).toEqual(
                'text/plain'
            );
            ...
            // ... element is no longer present
            expect(
                category.resourceElements.filter(
                    (e) => e.name === 'test-by-cat-ref'
                ).length
            ).toEqual(0);
            expect(
                category.allResourceElements.filter(
                    (e) => e.name === 'test-by-cat-ref'
                ).length
            ).toEqual(0);
            // ... elements container properties have decreased size
