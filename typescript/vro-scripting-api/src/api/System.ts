namespace vroapi {
    ...
    export class System {
        ...
        static sleep(ms: number): boolean {
            ...
            // This implementation is CPU intensive but is the only way that is currently supported without promices.
            const dt = new Date().getTime();
	        while ((new Date().getTime()) - dt <= ms) { /* Do nothing */ }
            return true;
        }
        ...
        static waitUntil(waitDate: Date, milli: number): boolean {
            const targetSleep = waitDate && waitDate.getTime() - new Date().getTime();
            if (!targetSleep || targetSleep <= 0) {
                return false;
            }
            return this.sleep(targetSleep);
        }
