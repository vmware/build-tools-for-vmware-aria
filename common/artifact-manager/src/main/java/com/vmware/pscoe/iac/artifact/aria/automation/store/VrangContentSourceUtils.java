...
public class VrangContentSourceUtils {
	...
	public String syncContentSource(VraNgContentSourceBase contentSource, Integer timeout) {
		...
		try {
			...
			throw new RuntimeException(
					String.format("Synchronization of content source %s failed, error %s", id, e.getMessage()), e);
		}
		return id;
	}
	...
	private void waitForContentSourceSync(VraNgContentSourceBase contentSource, String name, Integer timeout) {
		...
		try {
			logger.info("Waiting (max {} ms) content source '{}' ('{}') to be synchronized on target system", timeout,
	...
	private boolean isContentSourceSynced(String contentSourceId) {
		...
		if (contentSource.getLastImportErrors() != null && !contentSource.getLastImportErrors().isEmpty()) {
			String message = String.format("Unable to synchronize content source '%s' due to '%s'", contentSourceId,
					contentSource.getLastImportErrors().toString());
		...
		// Blueprint content sources require another call to update their items details
		// for some reason...
	...
	public File storeContentSourceOnFilesystem(VraNgContentSourceBase contentSource) {
		...
		File contentSourceFile = Paths
				.get(store.getPath(), VraNgDirs.DIR_CONTENT_SOURCES, contentSource.getName() + ".json").toFile();
