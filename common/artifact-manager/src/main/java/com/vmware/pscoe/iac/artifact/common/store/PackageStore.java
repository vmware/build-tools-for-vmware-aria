...
/**
 * An interface representing working with packages from the view point of the
 * server that consumes such packages.
 *
 * In that sense exporting a packaging is the process that includes contacting
 * the server and downloading a package
 * file from it into a local file/folder structure.
 *
 * On the contrary, importing a packaging means to take some existing
 * file/folder structure and upload it into the
 * server, where the server would take appropriate actions to properly apply the
 * package.
 *
 * Locally, the package is represented by an {@link Package} object.
 * 
 * @param <T> Could be different for different sort of servers (vRA, vRO, vROps,
 *            etc...).
 */
public interface PackageStore<T extends PackageDescriptor> {
	 ...
	 */
	List<Package> deleteAllPackages(List<Package> vroPackages, boolean lastVersion, boolean oldVersions,
			boolean dryrun);
