 ...
 * #L%
 */
package com.vmware.pscoe.iac.artifact.aria.logs.rest.v2;
...
import com.vmware.pscoe.iac.artifact.aria.logs.rest.AbstractRestClientVrli;
import com.vmware.pscoe.iac.artifact.aria.logs.rest.Errors;
...
import com.vmware.pscoe.iac.artifact.aria.logs.rest.v2.models.ContentPackMetadataListDTO;
...
public class RestClientVrliV2 extends AbstractRestClientVrli {
	...
	public void deleteAlert(final String alertId) {
		...
		try {
			...
			throw new RuntimeException(
					String.format("Unable to determine REST endpoint for deleting alert, error: %s", e.getMessage()),
					e);
		}
		...
		if (!HttpStatus.OK.equals(response.getStatusCode())) {
			throw new RuntimeException(String.format(
					"Unable to delete alert, Remote REST service returned status code %s, with message %s",
	...
	public void insertAlert(String alertJson) {
		...
		try {
			...
			throw new RuntimeException(
					String.format("Unable to determine REST endpoint for alert, error: %s", e.getMessage()), e);
		}
		if (!HttpStatus.CREATED.equals(response.getStatusCode())) {
			throw new RuntimeException(String.format(
					"Unable to insert/update alert, Remote REST service returned status code %s, with message %s",
					response.getStatusCode(), response.getBody()));
		}
	}
	...
	private void rewriteVcopsIntegrationInfo(final AlertDTO alert) {
		...
		try {
			...
			throw new RuntimeException(
					String.format(
							"Unable to update vCOPs integration for alert '%s', unable to fetch vROPs resources for adapter type '%s': %s",
		...
		Optional<ResourcesDTO.ResourceList> targetResource = resourceDto.getResourceList().stream()
				.filter(item -> item.getResourceKey().getResourceKindKey().equalsIgnoreCase(alertResourceType))
				.findFirst();
		...
		List<String> identifiersList = targetResource.get().getResourceKey().getResourceIdentifiers().stream()
				.map(identifier -> {
		...
		logger.info("Rewriting the vCopsResourceKindKey for alert '{}', using resource name '{}'", alert.getName(),
				resourceName);
		alert.getRecipients().getVrops().setVcopsResourceName(resourceName);
		alert.getRecipients().getVrops()
				.setVcopsResourceKindKey(baseResourceKindKeys.stream().collect(Collectors.joining("&")));
	}
	...
	private String getAdapterTypeForAlert(final AlertDTO alert) {
		...
		Optional<String> adapterTypeKeys = keysSplit.stream().filter(item -> item.contains(ADAPTER_KIND_KEY))
				.findFirst();

		if (!adapterTypeKeys.isPresent()) {
			throw new RuntimeException(String.format(
					"Unable to find adapter type '%s' for vROPs enabled alert: '%s' on the target vROPs system",
		...
		if (adapterTypes.isEmpty()) {
			throw new RuntimeException(String.format(
					"Unable to to extract vROPs adapter kind type for vROPs enabled alert '%s'", alert.getName()));
		}
	...
	private String getResourceTypeForAlert(final AlertDTO alert) {
		String targetResourceKindKeys = alert.getRecipients().getVrops().getVcopsResourceKindKey();
		List<String> keysSplit = Arrays.asList(targetResourceKindKeys.split(KEYS_SPLIT_KEY));
		Optional<String> resourceTypes = keysSplit.stream().filter(item -> item.contains(RESOURCE_KIND_KEY))
				.findFirst();
		if (!resourceTypes.isPresent()) {
			throw new RuntimeException(String.format(
					"Unable to find resource type '%s' for vROPs enabled alert: '%s' on the target vROPs system",
					RESOURCE_KIND_KEY, alert.getName()));
		}
		List<String> types = Arrays.asList(resourceTypes.get().split(KEY_SPLIT_KEY));
		if (types.isEmpty()) {
			throw new RuntimeException(String.format(
					"Unable to to extract vROPs resource kind type for vROPs enabled alert '%s'", alert.getName()));
		}
	...
	private AlertDTO findAlertByName(final String alertName) {
		...
		List<AlertDTO> foundAlerts = alerts.stream().filter(alert -> alert.getName().equalsIgnoreCase(alertName))
	...
	private List<ContentPackDTO> deserializeContentPacks(final String contentPacksJson) {
		ObjectMapper mapper = new ObjectMapper();
		try {
			ContentPackMetadataListDTO contentPackMetadata = mapper.readValue(contentPacksJson,
					ContentPackMetadataListDTO.class);
