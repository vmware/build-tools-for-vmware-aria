...
public class SshPackageStore extends GenericPackageStore<BasicPackageDescriptor> {
    ...
    protected static final Logger logger = LoggerFactory.getLogger(SshPackageStore.class);

    protected final ConfigurationSsh config;
    ...
    protected void exportFiles(Package sshPackage, List<String> files) {
    ...
    protected void reconnect() throws JSchException {
        if (session != null && session.isConnected()) {
            return;
        }
        logger.info("SSH session is closed, trying to (re)connect");
        connect();
    }
