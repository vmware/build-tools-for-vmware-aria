...
if [ $uncommited_files -gt 0 ]; then
    ...
    mvn versions:set-property -Dproperty=revision -DnewVersion=$ver -DgenerateBackupPoms=false --non-recursive
    ...
    mvn versions:set-property -Dproperty=revision -DnewVersion=$NEW_VERSION -DgenerateBackupPoms=false --non-recursive
    mvn initialize -f vro-types
    mvn initialize -f typescript
