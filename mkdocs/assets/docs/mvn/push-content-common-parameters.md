...
=== "Directly Pass The Parameters"
    ...
    ```bash
    mvn package {projectGoal}:push -P{{ archetype.customer_project.maven_profile_name}} -Dvrealize.ssl.ignore.certificate
    ```
    ...
    ```bash
    mvn package {projectGoal}:push -P{{ archetype.customer_project.maven_profile_name}} -Dvrealize.ssl.ignore.hostname
    ```
